...
public class FatTrieImpl implements FatTrie {

    TrieImpl origTrie;
    SecureTrie secureTrie;

    public FatTrieImpl(KeyValueDataSource origTrieDS, KeyValueDataSource secureTrieDS) {
        origTrie = new TrieImpl(origTrieDS);
        secureTrie = new SecureTrie(secureTrieDS);
    }

    public FatTrieImpl(KeyValueDataSource origTrieDS, KeyValueDataSource secureTrieDS, byte[] rootHash) {
	this( origTrieDS, secureTrieDS );
	if ( rootHash != null ) this.setRoot( rootHash );
    }
